...
func (s *RedisStore) Set(key string, value string) error {
	...
	ExpireTime := time.Minute * time.Duration(config.GetInt64("captcha_expire_time"))
	// 方便本地开发调试
	if app.IsLocal() {
		ExpireTime = time.Minute * time.Duration(config.GetInt64("captcha_debug_expire_time"))
	}
