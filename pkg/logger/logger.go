...
func getEncoder() zapcore.Encoder {
	...
	encoderConfig := zapcore.EncoderConfig{
		...
		EncodeTime:     customTimeEncoder,              // 时间格式，我们自定义为 2006-01-01 13:03:33
		EncodeDuration: zapcore.SecondsDurationEncoder, // 执行时间，以秒为单位
		EncodeCaller:   zapcore.ShortCallerEncoder,     // Caller 短格式，如 type/converter.go:14，长格式为绝对路径
	}
...
func customTimeEncoder(t time.Time, enc zapcore.PrimitiveArrayEncoder) {
	enc.AppendString(t.Format("2006-01-01 13:03:33"))
}
...
// Dump 调试专用，不会中断程序，会在终端打印 warning 消息
// 第一个参数会使用 json.Marshal 进行渲染，第二个参数消息（可选）
//
//	logger.Dump(user.User{Name: "text"})
//	logger.Dump(user.User{Name: "text"}, "用户消息")
