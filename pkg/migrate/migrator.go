...
func (migrator *Migrator) createMigrationsTable() {
	...
	if !migrator.Migrator.HasTable(&migration) {
		...
		if err != nil {
			console.Exit("Create migration table error")
		}
	}
}
...
// readMigrationFiles 从文件目录读取文件，确保正确的时间排序
func (migrator *Migrator) readAllMigrationFiles() []MigrationFile {

	// 读取 database/migrations/目录下所有的文件
	// 默认是会按照文件名进行排序
	...
	for _, f := range files {
		...
		// 加个判断，确保迁移文件可用，再放进 migrationFiles 数组中
...
// runUpMigration 执行迁移，执行迁移的 up 方法
