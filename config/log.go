...
func init() {
	config.Add("log", func() map[string]interface{} {
		return map[string]interface{}{
			...
			// 日志级别，必须是以下这些选项:
			// "debug" -- 信息量大，一般调试时打开。系统模块详细运行的日志，例如 HTTP 请求、数据库请求、发送邮件、发送短信等。
			// "info" -- 业务级别的运行日志，例如 用户登录、用户退出、订单撤销等。
			// "warn" -- 感兴趣、需要引起关注的信息，例如 调试时候打印调试信息（命令行输出会有高亮）。
			// "error" -- 记录错误信息，Panic 或 Error。如 数据库连接错误、HTTP 端口被占用。一般生产环境使用的等级。
			...
			/*- --------------------- 滚动日志配置 ------------------------- */
			// 日志文件路径
			"filename": config.Env("LOG_NAME", "storage/logs/logs.lig"),
			// 每个日志文件保存的最大尺寸 单位 M
